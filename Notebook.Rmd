...
test_set_predictions <- test_set_predictions %>%
                        ...
                        mutate(se_Models    = sd(c_across(all_of(pred_cols))),
                               se_combined  = pmax(se_Model1, se_Models),   
...
label_txt <- sprintf("Flagged: %s / %s (%.2f%%)\nRule: max(SE_GAM, SE_Ensemble) > %.2f",
                     format(n_flagged, big.mark = ","),
                     format(n_total, big.mark = ","),
                     pct_flagged,
                     se_limit)

plot_uncertainties <- ggplot(test_set_predictions, aes(x = se_Model1, y = se_Models)) +
                      ...
                      geom_vline(xintercept = se_limit, linetype = "dotted", linewidth = 0.7) +
                      geom_hline(yintercept = se_limit, linetype = "dotted", linewidth = 0.7) +
                      annotate("label",
                               x = 0.075, 
                               y = 0.24,
                               label = label_txt,
                      ...
                      coord_equal(xlim = c(0, 0.25), 
                                  ylim = c(0, 0.25), 
                                  expand = FALSE) +
                      labs(x = expression(SE[GAM]~"(Model 1, link scale)"),
                           y = expression(SE[Ensemble]~"(link scale)")) 
