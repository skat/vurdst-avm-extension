...
get_residuals <- function(models, dataset) {
  ...
  residual_list <- future_lapply(seq_along(models), function(i) {
    ...
    df <- as.data.frame(list("sales_price" = sales_price,
                             "sales_date_numeric" = sales_date_numeric,
...
residual_df$type <- with(residual_df,
                         ifelse(test,
                         "Test error",
                         ifelse(validation,
                                "Validation error",
...
test_set_predictions <- bind_cols(test_set, pred_matrix)
saveRDS(test_set_predictions, "saved_files/test_set_predictions.rds")

pred_cols <- grep("^pred_", names(test_set_predictions), value = TRUE)
sigma_limit <- 0.1
...
n_total <- nrow(test_set_predictions)
n_flagged <- sum(test_set_predictions$above_limit)
...
plot_uncertainties <- ggplot(test_set_predictions, aes(x = sigma_Model1, y = sigma_Ensemble)) +
  geom_point(alpha = 0.1, size = 0.8, color = "#08306B") +
  geom_point(data = subset(test_set_predictions, above_limit),
  ...
  coord_equal(xlim = c(0, 0.25), ylim = c(0, 0.25), expand = FALSE) +
...
outliers <- test_set_predictions %>% filter(above_limit)
...
residual_df_basis <- get_residuals(models_basis, dataset)
residual_df_basis$k <- as.numeric(sub(".*_(\\d+)$", "\\1", residual_df_basis$Model))
residual_df_basis$Model <- sub("^Model", "Model ", sub("(_.*)$", "", residual_df_basis$Model))
saveRDS(residual_df_basis, "saved_files/residual_df_basis.rds")

performance <- residual_df_basis %>% 
               mutate(dataset_type = case_when(test ~ "Test",
                                               validation ~ "Validation",
                                               TRUE ~ "Training")) %>% 
