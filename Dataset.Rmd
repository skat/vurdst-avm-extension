...
grundsalg_pcl <- grundsalg %>%
                 filter(salg.ejendomkategori == "EjendomKategoriUbebyggetgrund",
                        ...
                        deljordstykke.plandata_plandata.specifikanvendelseskategori == "1110",
...
grundsalg_pcl <- grundsalg_pcl %>% filter(!is.na(vurinfo.adresse.koordinatoest),
                                          ...
                                          !is.na(vurinfo.udsigtslaengde_soe),
                                          !is.na(vurinfo.areal_samlet_skov),
                                          !is.na(deljordstykke.plandata_plandata.udstykstoerrelse),
                                          !is.na(deljordstykke.plandata_plandata.maksbebygpct))
...
ejendomssalg_pcl <- ejendomssalg_pcl %>%
                    filter(as.integer(format(as.Date(salg.koebsdato), "%Y")) >= bygning.opfoerelsesaar,
                           ...
                           delgrund.registeret_areal_uden_vej == delgrund.arealberegnet) #drop ejendomme med vejarealer
...
vurderingsejendomme_pcl <- vurderingsejendomme_pcl %>%
                           filter(enhed.areal_til_beboelse == bygning.samlet_boligareal,
                                  ...
                                  delgrund.registeret_areal_uden_vej > 0,
                                  delgrund.registeret_areal_uden_vej == vurinfo.registreretareal_fratrukket_vejareal,
                                  delgrund.registeret_areal_uden_vej == delgrund.arealberegnet)
...
grundsalg_pcl <- grundsalg_pcl %>% filter(salg.ialtkoebesum >= 1e+5 & salg.ialtkoebesum <= 1.5e+7,
                                          vurinfo.registreretareal_fratrukket_vejareal <= 3000,
                                          vurinfo.registreretareal_fratrukket_vejareal >= 100,
                                          deljordstykke.plandata_plandata.maksbebygpct > 20,
                                          deljordstykke.plandata_plandata.maksbebygpct < 50,
                                          delgrund.zone %in% c(1,2)) #kun byzone og landzone
                                  
#Dan rest-, etageareal og udstykninger. Grunde med vejarealer er frasorteret, saa det gaar godt at anvende vurinfo.registreretareal_fratrukket_vejareal
grundsalg_pcl <- grundsalg_pcl %>%  
                 mutate(etageareal = vurinfo.registreretareal_fratrukket_vejareal * deljordstykke.plandata_plandata.maksbebygpct / 100,
                        restareal = pmax(0, vurinfo.registreretareal_fratrukket_vejareal - etageareal),
                        antal_udstykninger = pmax(1, floor(vurinfo.registreretareal_fratrukket_vejareal / deljordstykke.plandata_plandata.udstykstoerrelse))) %>% 
                 filter(antal_udstykninger %in% c(1,2,3))
...
variables <- c(id = "vurinfo.vurderingsejendom_id",
               ...
               gross_floor_area = "etageareal",
               residual_floor_areal = "restareal",
               number_of_subdivisions = "antal_udstykninger",
               type = "type")
